    ...
    <div className="bg-white rounded-lg   flex flex-col items-center text-center  border border-gray-200">
      <div className="top-container w-full flex items-center  p-2 ">
        ...
        <div className="flex flex-col items-start ml-4 gap-2 w-full">
          <h3 className="text-[14px] text-black text-left font-[500]">{agent.user.firstname} {agent.user.lastname}</h3>
          <span className="text-[10px] text-[#00000080]">@{agent.user.username}</span>

        </div>

        <div className="flex items-center justify-end  w-full mt-8">
          <span
            className={`w-3 h-3 rounded-full ${agent.AgentStatus === "online" ? "bg-green-500" : "bg-gray-400"
              }`}
          ></span>
          <span className="ml-2 text-sm text-gray-500">{agent.AgentStatus}</span>
        </div>
      </div>
